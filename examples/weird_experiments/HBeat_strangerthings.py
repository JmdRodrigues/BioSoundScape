...
scl = [midiToHz(i) for i in EventScale(root="G#", scale="minorPenta", first=4, octaves=2, type=0)]
freq = TrigChoice(b, scl)
sine = Sine(freq=[freq, freq], mul=amp*.25).out()

g_sharp_freq = 277.183
car = [random.triangular(freq-2, freq+2) for i in range(100)]
rat = [random.choice([.25, .5, 1, 1.25, 1.5, 2]) for i in range(100)]
ind = [random.randint(2, 6) for i in range(10)]
fm = FM(carrier=car, ratio=rat, index=sf_player, mul=1/100).out()
